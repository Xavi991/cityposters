@if(isset($offersDescription[0]))
	...
	@elseif($offersDesign[0] === 4)<!--DISEÑO 4-->
	...
	@elseif($offersDesign[0] === 5)<!--DISEÑO 5-->

		<?php
			$cincuentaPorciento= $offersBefore[0] / 2;
			$precioFinal= $cincuentaPorciento + $offersBefore[0];
		?>
		...
		<span class="offer-description offer-price-2 offer-price-2-fifth-design">
			<span style="font-size: 15px;">1 UNIDAD:</span>
	 		<span class="guaranies">Gs</span>
	 		<span>{{number_format($offersBefore[0], 0, "", ".")}}</span>
	 	</span>
	 ...
	 @elseif($offersDesign[0] === 6)<!--DISEÑO 6-->

		<?php
			$SetentaPorciento= ($offersBefore[0] * 70) / 100;
			$precioFinal= $SetentaPorciento + $offersBefore[0];
		?>
